					...
					uint32_t flags)
{
	...
	if (adreno_dev && (!adreno_dev->drawctxt_active || 1))
...
adreno_recover_hang(struct kgsl_device *device)
{
	...
	if (context) {
		...
		if (adreno_context) {
			KGSL_CTXT_ERR(device,"Context that caused a GPU hang: %d\n", adreno_context->id);
